...
public abstract class FileBasedIncidentReporter : IIncidentReporter, IDisposable, IAsyncDisposable
{
	...
	private readonly string _validatedFilePath;
	...
	/// </summary>
	protected string FilePath => _validatedFilePath;
	...
	/// Thrown if <paramref name="filePath"/> contains path traversal sequences that would escape the intended directory.
	/// </exception>
		...
		IncidentAggregator? aggregator = null)
	{
		...
		_validatedFilePath = ValidateFilePath(filePath);
		_filePath = filePath;
		_append = append;
	...
	private static string ValidateFilePath(string filePath)
	{
		...
		string normalizedInputPath = Path.GetFullPath(filePath);
		...
		// This handles cases like "C:\Windows\System32" on Windows or "/etc/passwd" on Unix
		...
		for (var attempt = 0; ; attempt++)
		{
			try
			{
				if (truncate)
				{
					...
					await File.WriteAllLinesAsync(_validatedFilePath, header.Concat(lines), _encoding).ConfigureAwait(false);
				}
			else
				{
